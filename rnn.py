...
class RNN_model(nn.Module):
    ...
    def forward(self,sentence,is_test = False):
        ...
        # ???
        h0 = torch.zeros(2, batch_input.size(0), self.lstm_dim).to(batch_input.device)  # 初始隐藏状态
        c0 = torch.zeros(2, batch_input.size(0), self.lstm_dim).to(batch_input.device)  # 初始细胞状态
